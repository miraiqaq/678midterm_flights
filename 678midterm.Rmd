...
#   filter(OP_UNIQUE_CARRIER %in% train$OP_UNIQUE_CARRIER)
# 
# test$OP_UNIQUE_CARRIER <- as.factor(test$OP_UNIQUE_CARRIER)
# test$ARR_DEL15 <- as.factor(test$ARR_DEL15)
# test$DEP_DEL15 <- as.factor(test$DEP_DEL15)
...
## linear model
# train <- train[train$total_delay<=1440,]
# test <- test[test$total_delay<=1440,]
# m1 <- lm(total_delay ~ DAY_OF_MONTH + DAY_OF_WEEK + OP_UNIQUE_CARRIER + DEP_DEL15 + ARR_DEL15 +
#            AIR_TIME + WT01 + WT02 + AWND + TAVG + PRCP + TimeOfDay, data = train)
# summary(m1)
# plot(m1, which = 1)
# 
# par(mfrow= c(2,3))
...
# actuals_preds <- data.frame(cbind(actuals=test$total_delay, predicteds=m1.predict))
...
##logistic model
library(car)
m4 <- glm(ARR_DEL15 ~ DAY_OF_MONTH  + OP_UNIQUE_CARRIER  + WT01 + WT02 + AWND + TAVG + PRCP +
           DAY_OF_WEEK + TimeOfDay + DISTANCE, data = train,  family = binomial(link = "logit"))
summary(m4)
...
library(randomForest) 
random_forest <- randomForest(train[-7], train$ARR_DEL15, proximity = TRUE, importance = TRUE)
random_forest
random_forest_validation <- predict(random_forest, test)
